...
function normalizeCalendarParams(event, action) {
  const type = pickFirst(event.type, event.calType, event.cal_type, 'user');
  const ownerId = Number(pickFirst(event.ownerId, event.owner_id, event.OWNER_ID, 0));
...
function createCalendarWebhookHandler() {
  return async function handler(req, res) {
    ...
    try {
      ...
      const status = error && error.code === 'MISSING_EVENT_ID' ? 400 : 502;
